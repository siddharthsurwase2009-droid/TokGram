...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'notifications':
        return (
            <div className="w-full h-full flex flex-col items-center justify-center bg-white text-center p-4 animate-in fade-in">
                ...
                <h2 className="text-xl font-bold mb-2 text-black">Activity</h2>
    ...
    <FeedProvider>
      <div className="flex h-screen w-full bg-white text-black font-sans overflow-hidden">
        ...
        <div className="flex-1 relative h-full overflow-hidden md:pl-[240px] flex flex-col">
          ...
          {activeTab === 'home' && (
            <header className="absolute top-0 left-0 right-0 z-30 pt-4 pb-2 px-4 flex justify-between items-center pointer-events-none bg-gradient-to-b from-white/90 to-transparent min-h-[60px] md:left-[240px]">
              {/* Search / Discover Button */}
              <button 
                onClick={() => setActiveTab('discover')}
                className="pointer-events-auto p-2 rounded-full bg-gray-100 backdrop-blur-md text-black hover:bg-gray-200 transition shadow-lg border border-gray-200"
              >
                <Search className="w-5 h-5" />
              </button>

              {/* Center Toggle */}
              <div className="absolute left-1/2 transform -translate-x-1/2 pointer-events-auto flex items-center space-x-6 bg-white/40 backdrop-blur-md px-5 py-2 rounded-full border border-gray-200 shadow-lg">
                <button 
                  onClick={() => setHomeViewMode('feed')}
                  className={`text-sm font-bold transition-all duration-300 ${homeViewMode === 'feed' ? 'text-black scale-105' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  Following
                </button>
                ...
                <button 
                  ...
                  className={`text-sm font-bold transition-all duration-300 ${homeViewMode === 'reels' ? 'text-black scale-105' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  For You
                </button>
              </div>
