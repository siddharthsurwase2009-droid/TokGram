...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange, onOpenCreate }) => {
  const NavItem = ({ id, icon: Icon, label }: any) => (
    <button
      ...
      className={`flex items-center space-x-4 p-3 w-full rounded-xl transition-all duration-200 group ${
        activeTab === id ? 'font-bold text-white' : 'text-gray-400 hover:bg-gray-900 hover:text-white'
      }`}
    >
  ...
  return (
    <div className="hidden md:flex flex-col w-[240px] h-full fixed left-0 top-0 bg-black border-r border-gray-800 z-50 px-3 py-5">
      {/* Logo */}
      <div className="px-3 mb-8 pt-2 cursor-pointer" onClick={() => onTabChange('home')}>
        <h1 className="text-2xl font-bold bg-gradient-to-r from-accent to-neon bg-clip-text text-transparent" style={{ fontFamily: 'cursive' }}>
          TokGram AI
        </h1>
      </div>
      ...
      <div className="flex-1 space-y-2">
        ...
        <div onClick={onOpenCreate} className="flex items-center space-x-4 p-3 w-full rounded-xl transition-all duration-200 text-gray-400 hover:bg-gray-900 hover:text-white cursor-pointer group">
