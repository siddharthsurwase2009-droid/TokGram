...
const Stories: React.FC = () => {
  ...
  return (
    <div className="flex gap-4 overflow-x-auto no-scrollbar py-4 px-4 bg-black border-b border-gray-800">
      {stories.map((story) => (
        <div key={story.id} className="flex flex-col items-center space-y-1 min-w-[72px] cursor-pointer group">
          <div className={`relative p-[3px] rounded-full transition-transform transform group-hover:scale-105 ${story.isUser ? '' : 'bg-gradient-to-tr from-yellow-400 to-fuchsia-600'}`}>
            <div className="w-16 h-16 rounded-full border-2 border-black overflow-hidden relative bg-gray-900">
              <img src={story.img} alt={story.name} className="w-full h-full object-cover" />
               {story.isUser && (
                <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                     <div className="bg-blue-500 rounded-full p-1 absolute bottom-0 right-0 border-2 border-black">
          ...
          <span className="text-[11px] text-gray-300 w-full text-center truncate px-1">{story.name}</span>
        </div>
      ))}
    </div>
  );
